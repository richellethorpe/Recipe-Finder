export default class RecipeFinder {
  static async getRecipes(food, mealType) {
    try {
      ...
      const response = await fetch(`https://api.edamam.com/api/recipes/v2?type=public&q=${food}&app_id=d760aafd&app_key=${process.env.API_KEY}&mealType=${mealType}&random=true`);
